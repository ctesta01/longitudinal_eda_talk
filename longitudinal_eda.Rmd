...
<a style='color: #55acee;' href='https://twitter.com/_christiantesta'>`r icons::icon_style(icons::fontawesome("twitter"), fill='#55acee')` @_christiantesta</a> <br>
...
ggplot(data = df,  aes(x = year, y = score, group = id, color = strata)) +
  geom_line(alpha=0.5) + 
  facet_wrap(~strata) #<<
```
]
...
ggplot(data = df,  aes(x = year, y = score, group = id, color = gender)) +
  geom_line(alpha=0.5) + 
  facet_wrap(~strata) + 
  ...
  ggtitle("Men have higher scores than women")
```
]
...
ggplot(data = df,  aes(x = year, y = score, color = gender)) +
...
df %>% group_by(strata, gender, year) %>%
  summarize(
    ci_high = quantile(score, 0.975),
    ci_low = quantile(score, 0.025),
    mean = mean(score),
    .groups = 'keep') %>%
...
```{r warning=F}
df_wide <- df %>% 
  tidyr::pivot_wider(id_cols = -score, names_from = year, values_from = score)
